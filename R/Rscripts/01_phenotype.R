...
##TEMPORARY Adding column to count the number of identical entries (potential duplicates)
#warf_df <- warf_df %>%
#group_by(eid, issue_date, dosis, quantity) %>%
#mutate(n_identical_on_date = length(eid))
...
# Creating df including eid below 1% quantile and above 99% quantile 
#outliers <- warf_df %>% 
#  filter(avg_daily_dosis < quantile(avg_daily_dosis, 0.01) | 
#           avg_daily_dosis > quantile(avg_daily_dosis, 0.99)) %>%
#  select(eid)
...
hist(warf_df$avg_daily_dosis, ylim=range(0,0.8), main="Histogram of response variable", xlab="log(dose)", ylab = "density", prob=TRUE, breaks=50)
