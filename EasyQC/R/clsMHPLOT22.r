...
setMethod("setMHPLOT", signature = (object = "MHPLOT"), function(object) {
	...
	aEqcSlotNamesIn = c("colMHPlot","colInChr","colInPos",
						...
						"fileAnnot","numAnnotPosLim","numAnnotPvalLim","blnPlotVerticalLines")
...
MHPLOT.GWADATA.valid <- function(objMHPLOT, objGWA) {
	...
	if(objMHPLOT@fileAnnot != "") {
		...
		### Cols exist?
			...
			if(!isAv)
				stop(paste(" EASY ERROR:MHPLOT\n Column 'Colour' or 'Exclude' are not available in fileAnnot. PLease correct file header.", sep=""))	
...
MHPLOT.run <- function(objMHPLOT, objGWA) {
	...
	if(fileAnnot != "") {
		...
		aisExclude = rep(FALSE,nrow(tblPlot))
		...
		for(iLocus in 1:nrow(tblAnnot)) {
			...
			if(any(isOverlap)) {
				...
				if(isUseExclude) {
					...
					aisExclude[isLocus] = isLocusExclude
				}
			}
		}
		if(isUseExclude) {
			tblPlot = tblPlot[!aisExclude,]
			warning(paste("EASY WARNING:\n There are ",length(which(aisExclude)), " SNPs being excluded from the MH plot due to Exclude column in \n ", fileAnnot ,sep="" ))
		}
	}
