...
METAANALYSIS.run <- function(objMA, objGWA) {
	...
	if(objMA@strMethod == "FIXEDEFFECT") {
		...
		for(i in 1:nDf) {
			...
			aBeta 	<- GWADATA.getcol(objGWA,objMA@acolBETAs[i])
			aSe 	<- GWADATA.getcol(objGWA,objMA@acolSEs[i])
			if(class(aBeta) != "numeric" & class(aBeta) != "double" )
				stop(paste("EASY ERROR:METAANALYSIS\nColumn \n",objMA@acolBETAs[i],"\nis not a numeric variable in file \n",objGWA@fileIn,"\n!!!" ,sep="" ))
			if(class(aSe) != "numeric" & class(aSe) != "double" )
				stop(paste("EASY ERROR:METAANALYSIS\nColumn \n",objMA@acolSEs[i],"\nis not a numeric variable in file \n",objGWA@fileIn,"\n!!!" ,sep="" ))
			...
			if(isUseN) {
				aN 	<- GWADATA.getcol(objGWA,objMA@acolNs[i])
			...
			if(isUseAlleles) {
				...
				if(i == 1) {
					a1ref <- GWADATA.getcol(objGWA,objMA@acolA1s[1])
					a2ref <- GWADATA.getcol(objGWA,objMA@acolA2s[1])
				} else {
					a1in <- GWADATA.getcol(objGWA,objMA@acolA1s[i])
					a2in <- GWADATA.getcol(objGWA,objMA@acolA2s[i])
		...
		for(i in 1:nDf) {
			...
			aZ 	<- GWADATA.getcol(objGWA,objMA@acolZSCOREs[i])
			aN 	<- GWADATA.getcol(objGWA,objMA@acolNs[i])
			...
			if(class(aZ) != "numeric" & class(aZ) != "double" ) 
				stop(paste("EASY ERROR:METAANALYSIS\nColumn \n",objMA@acolZSCOREs[i],"\nis not a numeric variable in file \n",objGWA@fileIn,"\n!!!" ,sep="" ))
			if(class(aN) != "numeric" & class(aN) != "double" & class(aN) != "integer") 
				stop(paste("EASY ERROR:METAANALYSIS\nColumn \n",objMA@acolNs[i],"\nis not a numeric variable in file \n",objGWA@fileIn,"\n!!!" ,sep="" ))
