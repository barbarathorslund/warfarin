...
CALCPHET.run <- function(objCP, objGWA) {
	...
	if(objCP@strMethod == "BETA") {
		acolBETAs 	<- objCP@acolBETAs
		acolSEs 	<- objCP@acolSEs	
		
		nDf = length(acolBETAs)
		...
		for(i in 1:nDf) {
			...
			aBeta 	<- GWADATA.getcol(objGWA, acolBETAs[i])
			aSe 	<- GWADATA.getcol(objGWA, acolSEs[i])
		...
		acolZSCOREs <- objCP@acolZSCOREs
		acolNs 		<- objCP@acolNs	
		
		nDf = length(acolZSCOREs)
		...
		for(i in 1:nDf) {
			...
			aZ 	<- GWADATA.getcol(objGWA, acolZSCOREs[i])
			aN 	<- GWADATA.getcol(objGWA, acolNs[i])
