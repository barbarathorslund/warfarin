...
setMethod("setCLUMP", signature = (object = "CLUMP"), function(object) {
	...
	aEqcSlotNamesIn = c("rcdCriterion","colInMarker","colClump","numPosLim","numR2Lim","filePLINK","fileBfile","blnAddClumpInfo","blnRemovePlinkFiles","strTag")
...
CLUMP.run <- function(objCLUMP, objGWA, objREPORT) {
	...
	##Use everything that survives the criterion:
	numPvalLim 		<- 1 
	...
	blnRemovePlinkFiles <- objCLUMP@blnRemovePlinkFiles
	...
	# objGWA.clumpout@tblGWA 		<- as.data.table(tblOutAll)
	# objGWA.clumpout@aHeader 	<- names(tblOutAll)
	# objGWA.clumpout@aClasses	<- sapply(tblOutAll,class)

	if(nchar(strTag)>0) {
		strTag = paste(strTag,".",sep="") 
		names(tblOutAll)[2:ncol(tblOutAll)] <- paste(strTag,names(tblOutAll)[2:ncol(tblOutAll)],sep="")
	}

	objGWA.clumpout <- GWADATA()
	objGWA.clumpout <- GWADATA.settbl(objGWA.clumpout,tblOutAll)
	...
	# objGWA.clump.x 	<- GWADATA.getrows(objGWA.clump, which(!is.na(objGWA.clump@tblGWA[,paste(strTag,"aTopHit",sep="")])))
	objGWA.clump.x 	<- GWADATA.getrows(objGWA.clump, which(!is.na(GWADATA.getcol(objGWA.clump,paste(strTag,"aTopHit",sep="")))))
	objGWA.clump.x 	<- GWADATA.removecols(objGWA.clump.x, c("KB","RSQ"))
	# objGWA.clump.nomatch <- GWADATA.getrows(objGWA.clump, which(is.na(objGWA.clump@tblGWA[,paste(strTag,"aLociTag",sep="")])))	
	objGWA.clump.nomatch <- GWADATA.getrows(objGWA.clump, which(!is.na(GWADATA.getcol(objGWA.clump,paste(strTag,"aLociTag",sep="")))))
	...
	if(blnAddClumpInfo) {
		...
		acolTmp <- c(colInMarker, paste(strTag,c("aLociTag", "aTopHit", "aNumLocusSNPs"),sep=""))
		objGWA.clump.tmp <- GWADATA.getcols(objGWA.clump, acolTmp)
