...
setMethod("setQQPLOT", signature = (object = "QQPLOT"), function(object) {
	...
	if(length(objEqcReader@lsEqcSlotsOut) > 0) {
		for(i in 1:length(objEqcReader@lsEqcSlotsOut)) {
			...
			if(tmpSlot == "arcdInclude") tmpSlotVal[is.na(tmpSlotVal)]=""
...
validQQPLOT <- function(objQQPLOT) {
	...
	if(!(objQQPLOT@strFormat %in% c("png","pdf","tiff","jpeg")))
...
QQPLOT.run <- function(objQQPLOT, objGWA, objREPORT) {
	...
	arcdInclude 		<- objQQPLOT@arcdInclude
	...
	blnAddLambdaGC		<- objQQPLOT@blnAddLambdaGC
	strPlotName			<- objQQPLOT@strPlotName
	anumChiSqDf			<- objQQPLOT@anumChiSqDf
	if(length(anumChiSqDf)==1) {
		if(anumChiSqDf==-1) {
			anumChiSqDf <- rep(1,length(acolQQPlot))
		}
	}
	if(length(anumChiSqDf) != length(acolQQPlot)) 
		stop(paste(" EASY ERROR:QQPLOT\n Length of 'anumChiSqDf' is unequal length of acolQQPlot!!!! PLease correct.", sep=""))
	...
	numXlimMax = 0
	numClean = 0
	for(iTmp in 1:length(aicolQQ)) {
		icolQQ = aicolQQ[iTmp]
		pTmp = tblPlot[,icolQQ]

		rcdExclude <- arcdExclude[iTmp]
		if(is.na(rcdExclude)) rcdExclude <- ""
		if(rcdExclude!="") {
			objRCD 	<- RCD(rcdExclude)
			out 	<- RCD.eval(objRCD, objGWA)
			if(any(out)) pTmp <- pTmp[!out]
		}
		rcdInclude <- arcdInclude[iTmp]
		if(is.na(rcdInclude)) rcdInclude <- ""
		if(rcdInclude!="") {
			objRCD 	<- RCD(rcdInclude)
			out 	<- RCD.eval(objRCD, objGWA)
			if(any(out)) pTmp <- pTmp[out]
			else pTmp <- rep(NA,length(pTmp))
		}
		if(blnCombined) numClean = numClean + length(which(!is.na(pTmp)))
		else numClean = max(numClean, length(which(!is.na(pTmp))))
	...
	# }
	# else {
		# for(icolQQ in aicolQQ) numClean = max( numClean, length(which(!is.na(tblPlot[,icolQQ]))))
		# numXlimMax = -log10(1/numClean)
	# }
	# plot.xlims = c(0, numXlimMax)
	# # old!
	...
	numSlopeID = 1
	#blnYAxisBreak = TRUE
	anumLambda <- rep(NA,length(acolQQPlot))
	...
	for(i in 1:length(acolQQPlot)) {
		...
		if(rcdInclude!="") {
			...
			if(any(out)) {
				...
				warning(paste("EASY WARNING:\n No SNPs fullfilled the criterion \n",rcdInclude, "\n. in  arcdInclude for file \n ", objGWA@fileIn ,sep="" ))
				aPobs <- rep(NA,length(aPobs))
			}
		}		###
		...
		if(i == 1 & blnPlotCI) {
			...
			for(iCi in 1:N_SNPS){
				...
				yCiUppLog[iCi] <- -log10(qbeta(0.975,iCi,N_SNPS-iCi+1))
				yCiDownLog[iCi] <- -log10(qbeta(0.025,iCi,N_SNPS-iCi+1))
			}
		}
		if(blnAddLambdaGC) {
			# aPobsCleanSortLog
			pgc <- 10^(-aPobsCleanSortLog)
			chi2gc <- qchisq(pgc,df=1,lower.tail=F)
			anumLambda[i] <- median(chi2gc,na.rm=T)/qchisq(0.5, df=1)
			rm(pgc)
			rm(chi2gc)
		}
	...
	if(blnAddLambdaGC) {
		...
		ytext = plot.ylims[2]
		yoffset = 1
		# if(nchar(strPlotName)>0) strPlotName = paste(strPlotName,".",sep="")
		
		for(i in 1:length(anumLambda)) {
			text(xtext,ytext,labels=paste("GC = ",round(anumLambda[i],3)),cex=1.5*plot.cex.main, col=astrColour[i], adj = c(0,yoffset))
			yoffset = yoffset + 1.2
			
			strNewReportColumn = paste(strPlotName,".GCLambda.",acolQQPlot[i],sep="")
			if(strNewReportColumn %in% names(objREPORT@tblReport)) {
				icolrep = 1
				strNewReportColumnTmp = strNewReportColumn
				while(strNewReportColumnTmp %in% names(objREPORT@tblReport)) {
					strNewReportColumnTmp = paste(strNewReportColumn,".",icolrep,sep="")
					icolrep = icolrep + 1
				}
				strNewReportColumn = strNewReportColumnTmp
			}
			objREPORT <- REPORT.addval(objREPORT,strNewReportColumn,anumLambda[i])
		}
	}
